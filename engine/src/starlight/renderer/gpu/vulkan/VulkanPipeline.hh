...
public:
    ...
    explicit VulkanPipeline(
      VkDevice device, Allocator* allocator, VulkanRenderPass& renderPass,
      Properties props
    ...
    ~VulkanPipeline() {
        if (m_handle) vkDestroyPipeline(m_device, m_handle, m_allocator);
        if (m_layout) vkDestroyPipelineLayout(m_device, m_layout, m_allocator);
    }
