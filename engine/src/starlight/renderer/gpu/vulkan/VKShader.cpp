...
//       m_device.getHandle(), &m_moduleCreateInfo, m_context.getAllocator(),
//       &m_handle
//     ));
...
//     m_stageCreateInfo.sType  =
//     VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO; m_stageCreateInfo.stage
//     = getStageFlagBits(props.type); m_stageCreateInfo.module = m_handle;
...
// ) :
//     Shader(props), m_context(context), m_device(device),
...
//     LOG_INFO(
//       "Shader resource - global samplers: {}, global uniforms: {}, instance
//       samplers: {}, instance uniforms: {}, local uniforms: {}",
...
//             descriptorSetConfig.bindings[bindingIndex].pImmutableSamplers =
...
//               VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
//             descriptorSetConfig.bindings[bindingIndex].pImmutableSamplers =
//             nullptr; descriptorSetConfig.samplerBindingIndex = bindingIndex;
//             bindingIndex++;
//         }
...
//         ASSERT(
//           not m_uniforms.contains(uniformProps.name), "Uniform '{}' already
//           added", uniformProps.name
//         );
...
//           "Shader global texture count {} exceed maximum {}", globalTextureCount +
//           1, globalTextureCount
//         );
...
//         LOG_TRACE(
//           "Push constant range: {} - {}, total push constants: {}", r.offset,
//           r.size, m_pushConstantRangeCount
//         );
//     }
...
//     // ASSERT(globalSetBindingCount <= 1, "Global image samplers not supported
//     yet");
...
//             uboDescriptor.sType           =
//             VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET; uboDescriptor.dstSet =
//             objectDescriptorSet; uboDescriptor.dstBinding      = descriptorIndex;
...
//         LOG_WARN(
//           "Provided texture map range size doesn't match with instance texture
//           count, ignoring and setting all to default map"
//         );
...
//     allocateInfo.sType              =
//     VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO; allocateInfo.descriptorPool =
//     m_descriptorPool; allocateInfo.descriptorSetCount = 3;
...
//     LOG_INFO("Minimal uniform buffer offset alignment={}",
//     m_requiredUboAlignment);
...
//     VulkanBuffer::Properties bufferProps;
